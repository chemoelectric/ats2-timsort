  ...
  for (size_t i = 0; i != sz; i += 1)
    {
      ...
      for (size_t j = 0; j != VALUESZ - 1; j += 1)
        arr[i].value[j] = '0' + (rand () % 10);
      arr[i].value[VALUESZ - 1] = '\0';
    }
}
              ...
              size_t offset, size_t sz)
{
  for (size_t i = 0; i != sz; i += 1)
    if (gotten[i + offset].key != expected[i].key
        || strcmp (gotten[i + offset].value, expected[i].value) != 0)
      {
        printf ("mismatched results: \n");
        printf ("  expected[%zu] = { %d, %s }\n",
                i, expected[i].key, expected[i].value);
        printf ("  gotten[%zu]   = { %d, %s }\n",
...
test_sorting (size_t sz)
{
  ...
  timsort_to_array (gotten3 + sz - 1, gotten3, sz, sizeof (entry_t),
                    less_than);

  /* Overlap test. */
  copy_array (gotten4 + sz - 1, unsorted, sz);
  timsort_to_array (gotten4, gotten4 + sz - 1, sz, sizeof (entry_t),
                    less_than);

  /* Overlap test. */
  copy_array (gotten5 + (sz / 2), unsorted, sz);
  timsort_to_array (gotten5, gotten5 + (sz / 2), sz, sizeof (entry_t),
                    less_than);

#if 0
  for (size_t i = 0; i != sz; i += 1)
    printf ("expected[%zu]={%d,%s}  gotten1[%zu]={%d,%s}\n",
            i, expected[i].key, expected[i].value,
            i, gotten[i].key, gotten1[i].value);
#endif
#if 0
  for (size_t i = 0; i != sz; i += 1)
    printf ("expected[%zu]={%d,%s}  gotten2[%zu]={%d,%s}\n",
            i, expected[i].key, expected[i].value,
