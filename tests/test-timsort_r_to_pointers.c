...
static void
check_gotten (const entry_t *expected, void **gotten,
              size_t sz)
{
  typedef const entry_t t;
  typedef t *tptr;
  for (size_t i = 0; i != sz; i += 1)
    if (((tptr) gotten[i])->key != expected[i].key
        || ((tptr) gotten[i])->value != expected[i].value)
      {
...
test_sorting (size_t sz)
{
  ...
  void **gotten = malloc (sz * sizeof (void *));
  ...
  timsort_r_to_pointers (gotten, unsorted, sz, sizeof (entry_t),
                         less_than, &env);
  long double t22 = get_clock ();
  long double t2 = t22 - t21;
  check_env (sz, 1234);

  check_gotten (expected, gotten, sz);
